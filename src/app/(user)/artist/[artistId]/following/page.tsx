...
import { useGetArtistFollowingListQuery } from "@/redux/features/user/userManagement";
...
export default function ArtistFollowingPage() {
    const [page, setPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");
    const [pendingUserId, setPendingUserId] = useState<number | null>(null);
    const { artistId } = useParams() as { artistId: string };

    const { data, isLoading, isFetching } = useGetArtistFollowingListQuery({
        page,
        limit: PAGE_LIMIT,
        id: Number(artistId),
    });

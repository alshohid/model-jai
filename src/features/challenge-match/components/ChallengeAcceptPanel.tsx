...
export default function ChallengeAcceptPanel({
  offer,
  gameLogo,
  onAccept,
  isAccepted,
  ...
  return (
    <div className="relative min-h-[150px] rounded-[16px] border border-white/16 bg-[#101116]/95 px-2.5 pb-3 pt-4 shadow-[inset_0_0_24px_rgba(255,255,255,0.04)] sm:min-h-[190px] sm:px-4">
      {isAccepted ? (
        <div
          className={`absolute left-1/2 top-2 -translate-x-1/2 rounded-full border border-[#66d7ff]/45  px-4 py-0.5 text-[10px] font-black uppercase text-white shadow-[0_0_14px_rgba(80,198,255,0.58)] ${isAccepted ? "bg-green-600" : "bg-red-600"}`}
        >
          Accepted
        </div>
      ) : (

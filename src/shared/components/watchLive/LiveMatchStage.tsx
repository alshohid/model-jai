...
export default function LiveMatchStage({
    ...
    const isPortraitStream = mode === "portrait";
    const isLandscapeStream = mode === "landscape";
    const showTwitchPlayer =
        (isLandscapeStream || isPortraitStream) && Boolean(twitchChannel);
                ...
                {isLive && (
                    <div className="px-3 pb-3 pt-1 sm:px-4 sm:pb-4">
                        ...
                        >
                            ...
                            <div
                                className={cn(
                                    "relative px-2 pb-2 pt-1 sm:px-3",
                                    isPortraitStream
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[10px] font-medium text-white/70 sm:text-xs">
                                        {isPortraitStream ? "Vertical Stream" : "HD Stream"}
                                    </span>
                                </div>

                                {isPortraitStream && (
                                    <span className="w-full text-[11px] text-white/45">
                                        Portrait broadcast is optimized for phone viewing.
                                    </span>
                                )}
                            </div>
                            ...
                            <div
                                className={cn(
                                    ...
                                    isPortraitStream
                                        ? "mx-auto h-[min(76dvh,42rem)] w-auto max-w-full aspect-[9/16]"
                                        : "aspect-video w-full max-h-[78vh]"
                                )}
                            >
                                {showTwitchPlayer ? (
